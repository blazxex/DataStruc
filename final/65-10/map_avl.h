...
class map_avl
{
  ...
  public:
    typedef tree_iterator iterator;
     int countLessThan(const KeyT& inputKey) {
        return countLessThan(mRoot, inputKey);
    }
  ...
  public:
    ...
    iterator upper_bound(const  int &k){
    node *crr = mRoot;
    node *ans = nullptr;
    while(crr!= nullptr){
      ...
      int countLessThan(node* n, const KeyT& inputKey) {
        if (n == NULL) {
            return 0;
        }

        if (mLess(n->data.first, inputKey)) {
            // Count this node and all nodes in its right subtree, and move to the right subtree
            return 1 + countLessThan(n->left, inputKey) + countLessThan(n->right, inputKey);
        } else {
            // Move to the left subtree
            return countLessThan(n->left, inputKey);
        }
    }
};
